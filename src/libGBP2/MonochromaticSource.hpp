 ...
 * so \lambda = \lambda_0 / n.
 */
class MonochromaticSource
{
 private:
  /*
   * We only need to trace 2 of the three parameters frequency, wavelength, refractive
   * index. The user will most often want to set the wavelength, so it makes since to track that.
   * Since frequency does not depend on the media, it makes since to track that too. However, if user just sets the
   * wavelength, it makes sense to assume that we are in vaccuum, so we would need to
   * track whether or not the frequency has been set.
   *
   * Wavelength and frequency have not obvious default value, but refractive index does.
   *
   * So, we can either track wavelength and frequency, and an additional flag to track if
   * both have been set. Or we could track frequency and refractive index, with n = 1 as
   * the default.
   *
   * We are going to track the frequency and refractive index so we don't have to keep an
   * extra flag, or use an optional.
   *
   *
   */
 ...
 public:
  ...
  template<typename U>
  void
  setVacuumWavelength(quantity<U> a_wavelength)
  {
    m_frequency = quantity<t::Hz>(constants::speed_of_light / a_wavelength);
  }
  template<typename U = t::nm>
  quantity<U>
  getVacuumWavelength() const
  {
    return quantity<U>(constants::speed_of_light / m_frequency);
  }

  /**
   * Set the vacuum frequency of the wave. This will change the _refractive index_,
   * NOT the frequency.
   */
  template<typename U>
  void
  setWavelength(quantity<U> a_wavelength)
  {
  ...
  template<typename U>
  void
  setFrequency(quantity<U> a_frequency)
  {
